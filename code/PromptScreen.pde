class PromptScreen {
  ...
  void handleClick() {
    if (generateButton.isClicked()) {
      generatedImage = aiManager.generateOutline("draw me " + inputBox.userInput + " in coloring book/sheet format in black and white");
      //generatedImage = localImage;
      //println(inputBox.userInput);
      if (generatedImage != null) {
        coloringScreen = new ColoringScreen(generatedImage);
